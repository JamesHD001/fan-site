...
export default function AddFundsPage() {
  ...
  useEffect(() => {
    if (!token) return
    loadWallet().catch((error) => setStatus({ type: 'error', message: error.message })).finally(() => setLoading(false))
  }, [token, loadWallet])
  ...
  const startPayment = async (event) => {
    ...
    if (!Number.isFinite(numericAmount) || numericAmount < 1) {
      setStatus({ type: 'error', message: 'Enter an amount of at least $1.' })
      return
    }
  ...
  return (
    <main className="add-funds-page">
      <section className="add-funds-card">
        ...
        <form onSubmit={startPayment} className="add-funds-form">
          ...
          <button className="add-funds-submit" type="submit" disabled={processing}>{processing ? 'Processing…' : 'Continue to payment'}</button>
        </form>

        <p className="add-funds-note">Your payment is processed securely by Flutterwave. Your account balance is only updated after our server verifies the transaction.</p>
      </section>

      {payment && <FlutterwaveInline payment={payment} onSuccess={verifyPayment} onClose={handleClose} onError={(message) => { setPayment(null); setProcessing(false); setStatus({ type: 'error', message }) }} />}
    </main>
  )
}
