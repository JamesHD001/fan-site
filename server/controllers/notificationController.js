...
const getUnreadCount = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Unable to retrieve notification count." });
  }
};
...
const getMyNotifications = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, data: { notifications, unreadCount, pagination: { page, limit, total, pages: Math.ceil(total / limit) } } });
  } catch (error) {
...
const markAllAsRead = async (req, res) => {
  try {
    const result = await Notification.updateMany({ user: req.user._id, isRead: false }, { $set: { isRead: true, readAt: new Date() } });
    return res.status(200).json({ success: true, message: "All notifications marked as read.", data: { modifiedCount: result.modifiedCount } });
  } catch (error) {
...
module.exports = { getUnreadCount, getMyNotifications, markAsRead, markAllAsRead, deleteNotification };
