...
/*
 * Returns the user's active membership tier
 * (plan.minimumMeetingTier), or null.
 */
...
/*
 * Public: list active meeting types.
 */
...
const getMyBookings = async (req, res) => {
  try {
    const bookings = await Booking.find({
      ...
      .populate(
        "meetingType",
        "name slug duration price currency minimumMembershipTier"
      )
      .sort({ scheduledFor: -1 });
...
/*
 * Authenticated: initialize payment for a booking.
 * Enforces membership tier requirements and slot availability.
 */
const initializeBookingPayment = async (req, res) => {
  try {
    const { meetingTypeId, scheduledFor, notes } =
      req.body;
    ...
    if (!meetingTypeId || !scheduledFor) {
      return res.status(400).json({
        success: false,
        message:
          "Meeting type and preferred date/time are required.",
      });
    }
    ...
    if (Number.isNaN(slotDate.getTime())) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid date/time provided for the meeting.",
      });
    }
    ...
    const plan = await getActiveMembershipTier(
      req.user._id
    );

    const requiredTier =
      meetingType.minimumMembershipTier;

    if (!plan) {
      return res.status(403).json({
        success: false,
        message:
          "An active membership is required to book a meeting.",
      });
    }
    ...
    const { ngnAmount, exchangeRate } =
      await convertUsdToNgn(meetingType.price);
    ...
    console.error(
      "Initialize booking payment error:",
      error
    );
    ...
    return res.status(500).json({
      ...
      message:
        error.message ||
        "Unable to initialize booking payment.",
    });
  }
};
